...
public final class MainActivity extends Activity {
    ...
    static final class Draft {
        ...
        Set<String> packages=new LinkedHashSet<>();boolean[] features=new boolean[5];
    ...
    private View pageHeader(){
        ...
        String s;switch(page){case "short":s="Contenus courts";break;case "games":s="Groupe Jeux";break;case "individual":s=draft==null?"Application":draft.label;break;case "select":s="Choisir les applications";break;case "permissions":s="Les deux autorisations";break;case "reward":s="Ta récompense";break;default:s="Paramètres";}
    ...
    private LinearLayout counterCard(boolean game,long used,boolean known){
        LinearLayout c=card(this);pad(c,13,15,13,15);String label=game?"Jeux":"Contenus courts";boolean enabled=game?prefs.gamesEnabled()&&!prefs.gamePackages().isEmpty():prefs.shortEnabled()&&prefs.anyShortFeature();int cap=game?prefs.gamesLimitMinutes():prefs.shortLimitMinutes();int start=game?prefs.gamesStartMinute():prefs.shortStartMinute(),end=game?prefs.gamesEndMinute():prefs.shortEndMinute();
    ...
    private LinearLayout limitCard(boolean game){
        LinearLayout c=card(this);LinearLayout r=row(this);r.addView(icon(this,game?"game":"play",FOREST,23));TextView h=title(this,game?"Jeux":"Contenus courts",19);pad(h,9,0,0,0);r.addView(h,weight());boolean enabled=game?prefs.gamesEnabled():prefs.shortEnabled();r.addView(pill(this,enabled?"Actif":"En pause",enabled));c.addView(r);space(c,13);
        if(!game){c.addView(muted(this,"Instagram · Facebook · YouTube",12));space(c,8);}
    ...
    private void editor(boolean game,boolean individual){
        ...
        }else if(!individual){body.addView(title(this,"Sources surveillées",17));space(body,9);LinearLayout sources=card(this);pad(sources,14,3,14,3);for(int n=0;n<5;n++){final int index=n;Switch s=new Switch(this);s.setText(Prefs.FEATURE_LABELS[n]);s.setTextColor(INK);s.setTextSize(14);s.setChecked(draft.features[n]);s.setMinHeight(dp(this,49));s.setOnCheckedChangeListener((x,v)->draft.features[index]=v);sources.addView(s,lp(-1,-2));}body.addView(sources);space(body,10);body.addView(muted(this,"Les écrans de messages ne sont pas ciblés. Un Reel ouvert depuis un message peut cependant être compté.",12));}
        space(body,24);body.addView(button(this,"Enregistrer",true,()->saveDraft(game,individual)),lp(-1,-2));space(body,10);
        if(individual&&prefs.getAppRule(draft.pkg)!=null)body.addView(button(this,"Supprimer cette règle",false,()->new AlertDialog.Builder(this).setTitle("Supprimer la règle ?").setMessage(draft.label+" ne sera plus limitée séparément.").setNegativeButton("Annuler",null).setPositiveButton("Supprimer",(d,w)->{prefs.removeAppRule(draft.pkg);journal.settingsChanged();draft=null;navigate("limits");}).show()),lp(-1,-2));
    ...
    private void saveDraft(boolean game,boolean individual){
        ...
        else{prefs.setShortEnabled(draft.enabled);prefs.setShortLimitMinutes(draft.limit);prefs.setShortStartMinute(draft.start);prefs.setShortEndMinute(draft.end);for(int n=0;n<5;n++)prefs.setFeature(Prefs.FEATURES[n],draft.features[n]);}
    ...
    private LinearLayout permissionCard(boolean accessibility){
        boolean enabled=accessibility?PermissionUtils.isAccessibilityEnabled(this):PermissionUtils.hasUsageAccess(this);LinearLayout c=card(this);LinearLayout row=row(this);row.addView(icon(this,accessibility?"shield":"history",FOREST,27));LinearLayout text=col(this);pad(text,12,0,0,0);text.addView(title(this,accessibility?"Accessibilité":"Données d’utilisation",17));space(text,5);text.addView(muted(this,accessibility?"Reconnaître et bloquer Reels, Stories et Shorts.":"Compter le temps des jeux et des applications.",13));row.addView(text,weight());c.addView(row);space(c,15);c.addView(button(this,enabled?"Activé ✓ · Ouvrir":"Activer",!enabled,()->{
