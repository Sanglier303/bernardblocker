...
public class AuditRegressionTest {
    ...
    private boolean action(String value,boolean byId){AccessibilityNodeInfo r=ui.getRootInActiveWindow();try{return NodeWalker.any(r,1800,n->n.isVisibleToUser()&&n.isClickable()&&(byId?n.getViewIdResourceName()!=null&&n.getViewIdResourceName().endsWith("/"+value):n.getText()!=null&&value.contentEquals(n.getText()))&&n.performAction(AccessibilityNodeInfo.ACTION_CLICK));}finally{if(r!=null)r.recycle();}}
    ...
    @Test public void actualPinThenAdminActivationReturnsHomeWithoutAnotherPin()throws Exception {
        try(ActivityScenario<MainActivity> a=launch()){
            ...
            assertTrue("Android activation button",action("action_button",true)||action("Activate this device admin app",false)||action("Activate",false));
    ...
    @Test public void reducingQuotaDoesNotErasePreviouslyCountedTime(){
        Journal j=new Journal(c);long now=System.currentTimeMillis();j.addShort(now,600000);p.setShortLimitMinutes(1);j.addShort(now+1000,1000);
