...
public class HardeningRegressionTest {
 ...
 @Test public void legacyAdbTamperLockIsAutomaticallyRetired(){p.raw().edit().putBoolean("tamper_lock_v4",true).putString("tamper_reason_v4","Le débogage ADB est actif et peut contourner Bernard").putLong("tamper_at_v4",123L).commit();Prefs migrated=new Prefs(c);assertFalse(migrated.tamperLock());assertEquals("",migrated.raw().getString("tamper_reason_v4",""));assertEquals(0L,migrated.raw().getLong("tamper_at_v4",0L));}
 @Test public void unrelatedTamperLockIsNeverClearedByAdbMigration(){p.raw().edit().putBoolean("tamper_lock_v4",true).putString("tamper_reason_v4","Horloge modifiée").putLong("tamper_at_v4",123L).commit();Prefs migrated=new Prefs(c);assertTrue(migrated.tamperLock());assertEquals("Horloge modifiée",migrated.tamperReason());assertEquals(123L,migrated.tamperAt());}
 @Test public void diagnosticReportsAdbOnlyAsInformation()throws Exception{org.json.JSONObject permissions=new org.json.JSONObject(com.local.focusfence.util.DiagnosticReport.export(c)).getJSONObject("permissions");assertTrue(permissions.has("adbEnabled"));assertEquals(com.local.focusfence.util.PermissionUtils.isAdbEnabled(c),permissions.getBoolean("adbEnabled"));}
