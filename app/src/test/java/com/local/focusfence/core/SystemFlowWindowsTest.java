...
public class SystemFlowWindowsTest {
    ...
    private static SystemFlowWindows returned() {
        ...
        w.begin(PKG, ADMIN); w.observeAuthorized(PKG, ADMIN, 41); w.complete(1000);
        return w;
    }
    @Test public void onlyAnObservedReturnedActivityIsStale() {
        SystemFlowWindows w = new SystemFlowWindows();
        w.begin(PKG, ADMIN); w.observeAuthorized(PKG, ADMIN, 41);
        assertFalse(w.isCompleted(PKG, ADMIN, 41));
        w.complete(1000); assertTrue(w.isCompleted(PKG, ADMIN, 41));
    }
    @Test public void missingObservationNeverRetiresAWindow() {
        SystemFlowWindows w = new SystemFlowWindows(); w.begin(PKG, ADMIN); w.complete(1000);
        assertFalse(w.isCompleted(PKG, ADMIN, 41));
    }
    @Test public void otherPackageAndActivityCannotBeRecorded() {
        SystemFlowWindows w = new SystemFlowWindows(); w.begin(PKG, ADMIN);
        w.observeAuthorized("com.attacker", ADMIN, 41);
        w.observeAuthorized(PKG, PKG + ".SubSettings", 41); w.complete(1000);
        assertFalse(w.isCompleted(PKG, ADMIN, 41));
    }
    ...
    @Test public void cancelledLaunchCannotRetireAWindow() {
        SystemFlowWindows w = new SystemFlowWindows(); w.begin(PKG, ADMIN);
        w.observeAuthorized(PKG, ADMIN, 41); w.cancelPending(); w.complete(1000);
        assertFalse(w.isCompleted(PKG, ADMIN, 41));
    }
    ...
    @Test public void nextFlowDoesNotCompleteEarlierUnreturnedActivity() {
        SystemFlowWindows w = new SystemFlowWindows();
        w.begin(PKG, ADMIN); w.observeAuthorized(PKG, ADMIN, 41);
        w.begin(PKG, PKG + ".SubSettings"); w.complete(1000);
        assertFalse(w.isCompleted(PKG, ADMIN, 41));
    }
    @Test public void completedWindowMemoryIsBounded() {
        SystemFlowWindows w = new SystemFlowWindows();
        for (int n=0;n<9;n++) { w.begin(PKG, ADMIN); w.observeAuthorized(PKG, ADMIN, n); w.complete(1000+n); }
